...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  get _steps => [
    ...
    (
      title: 'Ligue o Atendente IA',
      sub: 'Ative e escreva as instruções (persona e regras).',
      done: _b('ai_enabled') && _b('has_instructions'),
      optional: false,
      go: 'Atendente IA',
      embedded: false,
    ),
    (
      ...
      sub: 'Horários, contato e o contexto da empresa.',
      done: _b('has_knowledge'),
      optional: false,
      go: 'Atendente IA',
      embedded: false,
    ),
    (
      ...
      sub: 'Tokens por PIX ou cartão, na aba Planos.',
      done: _b('has_credits'),
      optional: false,
      go: 'Planos',
      embedded: false,
    ...
    (
      title: 'Ações da IA (opcional)',
      sub: 'Buscas na sua API — ex.: 2ª via de boleto.',
      done: _b('has_actions'),
      optional: true,
      go: 'Atendente IA',
      embedded: false,
    ...
    (
      title: 'Convide a equipe (opcional)',
      sub: 'Crie logins para os atendentes.',
      done: ((_st?['team_size'] ?? 1) as num) > 1,
      optional: true,
      go: 'Usuários',
      embedded: false,
    ),
  ...
  Future<void> _send() async {
    ...
    if (!mounted) return;
    final ans = (r.ok && r.data is Map)
        ? ((r.data as Map)['answer'] ?? '').toString()
        : (r.message ?? 'Não consegui responder agora.');
    setState(() {
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Vamos deixar sua conta pronta em poucos passos — $doneCount de ${steps.length} concluídos.',
                        style: TextStyle(
                          fontSize: 14,
      ...
      child: Column(
        ...
        children: [
          for (var i = 0; i < steps.length; i++)
            _stepRow(steps[i], last: i == steps.length - 1),
        ],
      ),
    );
  }
  ...
  Widget _stepRow(
    ...
    return Container(
      ...
      // LayoutBuilder + largura EXPLÍCITA no bloco de texto: no CanvasKit web o
      // Expanded dentro de Row colapsa a ~0 e o texto renderiza na vertical.
      // SizedBox com largura fixa (calculada aqui) não sofre desse bug.
      child: LayoutBuilder(
        builder: (context, c) {
          final showBtn = !s.done;
          final textW = (c.maxWidth - 22 - 12 - (showBtn ? 8 + 112 : 0))
              .clamp(60.0, double.infinity);
          return Row(
            children: [
              Icon(
                s.done ? Icons.check_circle : Icons.radio_button_unchecked,
                color: s.done ? const Color(0xFF1F9D57) : Colors.grey.shade400,
                size: 22,
              ),
              const SizedBox(width: 12),
              SizedBox(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Pergunte qualquer coisa sobre configurar o HotZap — ex.: "como conecto meu número?".',
            style: TextStyle(fontSize: 12.5, color: Colors.grey.shade600),
          ),
          ...
          LayoutBuilder(
            builder: (context, c) {
              final fieldW = (c.maxWidth - 8 - 48).clamp(80.0, double.infinity);
              return Row(
                children: [
                  SizedBox(
                    width: fieldW,
