...
const userColumns = `id, account_id, full_name, email, phone, role, is_active, COALESCE(presence,'available'), created_at, updated_at, deleted_at, profile_id`
...
func scanUser(row interface{ Scan(...any) error }) (*models.User, error) {
	...
	if err := row.Scan(&u.ID, &accountID, &u.FullName, &u.Email, &u.Phone,
		&u.Role, &u.IsActive, &u.Presence, &u.CreatedAt, &u.UpdatedAt, &u.DeletedAt, &u.ProfileID); err != nil {
		return nil, err
	}
